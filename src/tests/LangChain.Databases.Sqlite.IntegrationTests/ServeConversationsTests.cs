...
namespace LangChain.Databases.Sqlite.IntegrationTests
{
    ...
    public class ServeConversationsTests
    {
        ...
        {
            SQLiteConversationRepository sQLiteConversationRepository = new SQLiteConversationRepository("Data Source=:memory:;");
            
            // setup
            var conversation = await sQLiteConversationRepository.CreateConversation("test");
            sQLiteConversationRepository.AddMessage(new StoredMessage()
            {
                Content = "message1",
